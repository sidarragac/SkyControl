...
<template>
  <!-- Success Message -->
  <SuccessMessageComponent :class="props.formType === 'edit' ? 'mx-8' : ''" :success-message="successMessage" />

  <!-- Error Message -->
  <ErrorMessageComponent :class="props.formType === 'edit' ? 'mx-8' : ''" :error-message="errorMessage" />
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="space-y-2">
          ...
          <input v-model="form.name"
            ...
            placeholder="Name" type="text" name="airlineName" id="airlineName" required />
        </div>

        <div class="space-y-2">
          <label class="text-sm text-primary-700 font-semibold" for="airlineCountry">Country</label>
          <select v-model="form.country"
            class="w-full bg-white-100 text-black-800 border border-neutral-100 rounded-lg p-3 text-sm focus:ring-primary focus:border-accent-500 focus:outline-none"
            name="airlineCountry" id="airlineCountry" required>
        ...
        <div class="space-y-2">
          <label class="text-sm text-primary-700 font-semibold" for="destinations">Destinations</label>
          <input v-model="form.destinations"
            class="w-full bg-white-100 text-black-800 border border-neutral-100 rounded-lg p-3 text-sm focus:ring-primary focus:border-accent-500 focus:outline-none"
            placeholder="List of cities separated by commas" type="text" name="destinations" id="destinations"
            required />
        </div>
      </div>
    </section>
